  ...
  class modelLogUploadSoal extends mysql_db {
    ...
    public function importSoal($data,$info) {
      ...
      for ($i=2; $i <= $arrayCount; $i++) { 
        ...
        $jawab_1              = trim(htmlentities(htmlspecialchars(strip_tags($data[$i][2]),ENT_QUOTES)));//trim($data[$i]["B"]," \t\n\r\0\x0B\xA0");
        $jawab_2              = trim(htmlentities(htmlspecialchars(strip_tags($data[$i][3]),ENT_QUOTES)));//trim($data[$i]["C"]," \t\n\r\0\x0B\xA0");
        $jawab_3              = trim(htmlentities(htmlspecialchars(strip_tags($data[$i][4]),ENT_QUOTES)));//trim($data[$i]["D"]," \t\n\r\0\x0B\xA0");
        $jawab_4              = trim(htmlentities(htmlspecialchars(strip_tags($data[$i][5]),ENT_QUOTES)));//trim($data[$i]["E"]," \t\n\r\0\x0B\xA0");
