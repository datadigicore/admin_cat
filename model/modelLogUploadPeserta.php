<?php
  ...
  class modelLogUploadPeserta extends mysql_db {
    ...
    public function importPeserta($data) {
      ...
      $string = "INSERT INTO `master_peserta`(`id_peserta`, `id_ujian`, `id_lokasi`, `id_ruangan`, `no_peserta`, `nama`, `nrp`, `jenkel`, `pkt`, `kesatuan`, `tempat_lahir`, `tgl_lahir`, `status_ujian`, `waktu_mulai`, `durasi_pengerjaan`, `tambahan_waktu`, `waktu_selesai`, `skor_total`, `keterangan`) VALUES";
      // $urut=1;
      for ($i=7; $i < $arrayCount; $i++) { 
        $id_peserta   ='';
        $id_ujian            = '';//trim($data[$i]["A"]," \t\n\r\0\x0B\xA0");
        $id_lokasi            = '';//trim($data[$i]["A"]," \t\n\r\0\x0B\xA0");
        $id_ruangan            = '';//trim($data[$i]["A"]," \t\n\r\0\x0B\xA0");
        ...
        $pkt              = trim($data[$i]["G"]," \t\n\r\0\x0B\xA0");
        $nrp              = trim($data[$i]["H"]," \t\n\r\0\x0B\xA0");
        $kesatuan              = trim($data[$i]["L"]," \t\n\r\0\x0B\xA0");
        $tempat_lahir              = trim($data[$i]["M"]," \t\n\r\0\x0B\xA0");
        $tgl_lahir              = trim($data[$i]["N"]," \t\n\r\0\x0B\xA0");
        $keterangan              = trim($data[$i]["O"]," \t\n\r\0\x0B\xA0");
        
        $string .= "('$id_peserta','$id_ujian','$id_lokasi', '$id_ruangan', '$no_ujian', '$nama', '$nrp', '$jenkel', '$pkt', '$kesatuan', '$tempat_lahir', '$tgl_lahir', '$status_ujian', '$waktu_mulai', '$durasi_pengerjaan','$tambahan_waktu','$waktu_selesai','$skor_total', '$keterangan'),";
       // $urut++;
      }
